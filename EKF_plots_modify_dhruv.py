...
def process_csv_with_ekf(csv_path):
    ...
    for idx, row in enumerate(rows):
        ...
        # === GET IMU DATA ===
        gyro_x = safe_float(row.get('imu_angular_vel_noisy_x', 0))
        gyro_y = safe_float(row.get('imu_angular_vel_noisy_y', 0))
        gyro_z = safe_float(row.get('imu_angular_vel_noisy_z', 0))
        accel_x = safe_float(row.get('imu_linear_acc_noisy_x', 0))
        accel_y = safe_float(row.get('imu_linear_acc_noisy_y', 0))
        accel_z = safe_float(row.get('imu_linear_acc_noisy_z', 0))
        ...
        odom_x = safe_float(row.get('odom_pose_noisy_x', 0))
        odom_y = safe_float(row.get('odom_pose_noisy_y', 0))
        odom_z = safe_float(row.get('odom_pose_noisy_z', 0))
        ...
        odom_vx = safe_float(row.get('odom_vel_noisy_x', 0))
        odom_vy = safe_float(row.get('odom_vel_noisy_y', 0))
        odom_vz = safe_float(row.get('odom_vel_noisy_z', 0))
...
def create_plots(results, ground_truth, save_prefix='ekf'):
    ...
    axes[0].plot(time, results['error_attitude'], 'b-', lw=1)
    axes[0].set_ylabel('Attitude\nError (deg)')
    axes[0].grid(True, alpha=0.3)

    vel_err = np.sqrt((est_vx - true_vx)**2 +
                      (est_vy - true_vy)**2)
    print(f"{np.mean(vel_err)=}")
    axes[1].semilogy(time, vel_err, 'g-', lw=1)
    axes[1].set_ylabel('Velocity\nError (m/s)')
    axes[1].grid(True, alpha=0.3)

    axes[2].plot(time, results['error_position'], 'r-', lw=1)
    ...
    axes[3].plot(time, total_cov, 'm-', lw=1)
...
def main():
    ...
    csv_path = "combined_data_30Hz_NOISY.csv"
    ...
    import pickle
    with open('ekf_results.pickle', 'wb') as f:
    ...
    # Plot
    ground_truth_df = pd.read_csv("combined_data_30Hz_NOISY.csv")
